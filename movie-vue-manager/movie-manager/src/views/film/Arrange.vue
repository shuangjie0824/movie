		...
		<el-dialog title="新增电影排片" :visible.sync="dialog" width="50%">
			...
			<el-form ref="form" :model="arrangement" label-width="80px">
				...
				<el-form-item label="电影票价">
					<el-input-number v-model="arrangement.price" :precision="2" :step="0.1" :max="999.99">
					</el-input-number>
				</el-form-item>
		...
		<el-table v-loading="loading"
			...
			<el-table-column label="放映类型">
				<template slot-scope="scope">
					<el-tag effect="dark" v-if="scope.row.type === '2D放映'" type="success" disable-transitions>2D 放映
					</el-tag>
					<el-tag effect="dark" v-if="scope.row.type === '3D放映'" type="primary" disable-transitions>3D 放映
					</el-tag>
				</template>
			</el-table-column>
			...
			<el-table-column width="320" align="right">
				...
				<template style="white-space: nowrap" slot-scope="scope">
					...
					<el-popconfirm confirm-button-text='确定' cancel-button-text='不用了' icon="el-icon-info"
						icon-color="red" @confirm="handleDelete(scope.$index, scope.row)" title=" 确定要删除此电影档期吗？ ">
						<el-button style="margin-left: 8px" size="mini" icon="el-icon-delete" type="danger"
							slot="reference">删除
						</el-button>
					</el-popconfirm>
				</template>
